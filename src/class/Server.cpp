...
void Server::respond(int client_fd)
{
	...
	std::string content_length_str = parseHttpRequest(request, method, path);

	// If POST request, read the body
	if (method == "POST" && !content_length_str.empty()) {
		size_t content_length = static_cast<size_t>(std::stoi(content_length_str));
		while (body.length() < content_length &&
			(bytes_read = recv(client_fd, buffer, sizeof(buffer), 0)) > 0) {
